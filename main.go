...
func printExampleUsage() {
	fmt.Println("httpstat -4 -X GET https://www.google.com")
}
...
func init() {
	...
	flag.BoolVar(&example, "e", false, "show example usage")
	flag.IntVar(&count, "c", 1, "how many requests to make")
...
func printHeader(url *url.URL)  {

		scheme := url.Scheme
		response, err := http.Get(scheme + "://" + url.Host)
		if err != nil {
				log.Fatal(err)
		}

		// print status line and headers
		printf("\n%s%s%s\n", color.GreenString("HTTP"), grayscale(14)("/"), color.CyanString("%d.%d %s", response.ProtoMajor, response.ProtoMinor, response.Status))
		...
		for _, k := range names {
			printf("%s %s\n", grayscale(14)(k+":"), color.CyanString(strings.Join(response.Header[k], ",")))
		}
}
